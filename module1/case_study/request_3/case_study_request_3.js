function fAddInformation() {
    let name;
    do {
        let index, stringName = "", i = 1, j = 1, up;
        ...
        for (index = 0; index < nameLength; index++) {
            if (name.charAt(index) != " ") {
                if (j == 1) {
                    up = (name.charAt(index)).toLocaleUpperCase();
                }
                if (j == 1) {
                    stringName += up;
                } else {
                    up = (name.charAt(index)).toLocaleLowerCase();
                    stringName += up;
                }
                j = 2;
            } else {
                j = 1;
                if (name.charAt(index - i) != " " && name.charAt(index + 1) != " ") {
                    up =
                    stringName += name.charAt(index);
                    i = 1;
                } else {
                    i++;
                }
